...
import type { AnalyticsSummary, AttemptRecord, ScoreDistribution } from '@/app/admin/mock-tests/types/analytics'
import { subDays } from 'date-fns'

export default async function AdminAnalyticsPage({
  searchParams,
}: {
  searchParams: { testId?: string; groupId?: string; timeRange?: string }
}) {
  ...
  const { testId, groupId, timeRange } = searchParams;
  ...
  const [tests, groups] = await Promise.all([
    prisma.sATMockTest.findMany({ select: { id: true, name: true }, orderBy: { createdAt: 'desc' } }),
  ...
  const avgRWScore = attempts.length
    ? Math.round(attempts.reduce((sum, a) => sum + (a.rwScore || 0), 0) / attempts.length)
    : 0
  const avgMathScore = attempts.length
    ? Math.round(attempts.reduce((sum, a) => sum + (a.mathScore || 0), 0) / attempts.length)
    : 0
  ...
  const scoreDistribution: ScoreDistribution[] = scoreRanges.map((range) => {
    const count = attempts.filter(
      (a) => a.totalScore !== null && a.totalScore >= range.min && a.totalScore <= range.max
    ).length
    return {
      range: range.range,
      count,
      percentage: attempts.length ? Math.round((count / attempts.length) * 100) : 0,
    }
  })
  ...
  attempts.forEach(a => {
    if (a.completedAt) {
      ...
      completionsOverTimeMap.set(dateStr, (completionsOverTimeMap.get(dateStr) || 0) + 1);
    }
  });
      ...
      <div className="lg:ml-64">
        ...
        <main className="pt-24 px-6 pb-12">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Filters */}
            <div className="mb-6">
               <AnalyticsFilters tests={tests} groups={groups} currentTest={testId} currentGroup={groupId} currentTime={timeRange} />
            </div>

            {/* Summary Cards */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold mb-2">Total Attempts</p>
                <p className="text-3xl font-bold text-yellow-400">{totalAttempts}</p>
              </div>
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold mb-2">Avg Total Score</p>
                <p className="text-3xl font-bold text-blue-600">{avgTotalScore}</p>
              </div>
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold mb-2">Avg R&W</p>
                <p className="text-3xl font-bold text-emerald-600">{avgRWScore}</p>
              </div>
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold mb-2">Avg Math</p>
                <p className="text-3xl font-bold text-yellow-400">{avgMathScore}</p>
              </div>
            </div>
            ...
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-yellow-400 mb-4">Attempt History</h2>
