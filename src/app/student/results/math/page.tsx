...
export default async function MathResultsPage() {
  ...
  const avgScore =
    mathTests.length > 0
      ? Math.round(
          mathTests.reduce((sum: number, t) => sum + (t.score || 0), 0) /
            mathTests.length,
        )
      : 0;

  const highestScore =
    mathTests.length > 0 ? Math.max(...mathTests.map((t) => t.score || 0)) : 0;

  const lowestScore =
    mathTests.length > 0 ? Math.min(...mathTests.map((t) => t.score || 0)) : 0;

  const trendPercentage =
    mathTests.length > 1
      ? Math.round(
          (((mathTests[0].score || 0) -
            (mathTests[mathTests.length - 1].score || 0)) /
            (mathTests[mathTests.length - 1].score || 1)) *
            100,
        )
      : 0;
 ...
 percentage
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-600 dark:text-slate-400 font-medium">
                  No Math test results yet
                </p>
                <p className="text-slate-500 dark:text-slate-400 text-sm mt-2">
                  Take a Math mock test to see your results here
                </p>
                <Link
          ...
          >
            <p className="text-slate-600 dark:text-slate-400 mb-4">
              Want to view your English results?
            </p>
            <Link
              href="/student/results/english"
              className="inline-block px-6 py-3 bg-violet-600 hover:bg-violet-700 text-slate-900 dark:text-white rounded-lg font-medium transition-colors"
            >
